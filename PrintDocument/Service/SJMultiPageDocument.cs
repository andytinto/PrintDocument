    ...
    public sealed class SJMultiPageDocument : IDocument
    {
        ...
        private void ComposeContent(IContainer container, IReadOnlyList<SuratJalanItem> items)
        {
            container.Table(table =>
            {
                ...
                table.Header(h =>
                {
                    h.Cell().Border(1).AlignCenter().Text("NO").Bold();
                    h.Cell().Border(1).AlignCenter().Text("JENIS & UKURAN BARANG").Bold();
                    h.Cell().Border(1).AlignCenter().Text("JUMLAH").Bold();
                    h.Cell().Border(1).AlignCenter().Text("KET").Bold();
                    h.Cell().Border(1).AlignCenter().Text("P1/P2/SISA").Bold();
                });
                ...
                foreach (var item in items)
                {
                    ...
                    table.Cell().Element(CellBorderLR)
                        .PaddingLeft(TableCellPaddingLeft)
                        .Text(text =>
                        {
                            text.DefaultTextStyle(x => x.LineHeight(1.4f));

                            text.Span(item.NamaBarang ?? "")
                                .WrapAnywhere();
                        });

                    table.Cell().Element(CellBorderLR)
                        .AlignRight()
                        .PaddingRight(TableCellPaddingLeft)
                ...
                // garis bawah table di akhir dokumen
                table.Footer(f =>
                {
                    f.Cell().Element(CellBorderLRB).Text(string.Empty);
                    f.Cell().Element(CellBorderLRB).Text(string.Empty);
                    f.Cell().Element(CellBorderLRB).Text(string.Empty);
                    f.Cell().Element(CellBorderLRB).Text(string.Empty);
                    f.Cell().Element(CellBorderLRB).Text(string.Empty);
